...
class SortRule {
    ...
    constructor(key:string, direction:SORTDIRECTION=SORTDIRECTION.ASC, sortFn?:Function) {
        this.key = key
 ...
 * @param {Array} items 
 */
const multiSort = (sortRules:SortRule[], items:string[]|number[]) => {
    // helper method to detect if both elements are numeric
    const areNumeric = (a:any,b:any) => {
        let typeA = typeof a
        let typeB = typeof b
    ...
    const getSortFunction = (a:string|number, b:string|number) => {
    ...
    toBeSorted.sort((itemA:any, itemB:any) => {
        let result = 0
